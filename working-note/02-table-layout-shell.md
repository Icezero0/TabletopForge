...
**目标**：`/rooms/:id` 改为 PRD 五区 **静态布局 + 工具栏槽位**，区域可占位、可折叠，**不接** 地图/角色/绘制真实数据。本步结束后应「看得像跑团桌」，点工具无业务效果（除路由级治理入口）。
...
TopToolBar（工具按钮区）
├── TopLeftDock
│   ├── InGameCharacterList   ← 占位列表
│   └── ChatFloatingPanel     ← 占位/折叠框
├── MapViewport               ← 中央大区域（网格+比例尺 UI）
├── RightInspector
│   ├── InfoPanel             ← 空态/占位
│   └── PersonalMemo          ← 可收起占位
└── BottomAssetBar            ← 「添加地图」「添加角色」按钮占位
```
...
- [ ] 进入房间即见五区布局，与 `03` §4.2 示意图一致。
- [ ] 网格在中央区域常显。
- [ ] 顶栏工具可切换选中态；测距未实现且符合 PRD。
- [ ] 右栏备忘录可收起；InfoPanel 仅占位。
- [ ] PL 看不到「添加地图」类按钮（占位逻辑即可）。
