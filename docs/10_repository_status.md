...
房间页（`frontend/src/pages/room/RoomPage.vue`）已实现 **全屏地图 + 悬浮可收起面板**（`features/table/`）：`TableStage` + `MapViewport` 铺满视口；左上 `GovernanceDock`（成员/入房请求/设置 Tab）、左下聊天、顶部工具条、底部素材条、右侧信息/备忘录均为 `FloatingPanel`。地图导入、Token、绘制、场上角色库等业务仍待 Step 4–5；交互见 `03_frontend_design.md` §4.2。
...
| 跑团桌面 MVP（`01` §4.2） | **进行中**（Step 1–2 已落地：身份 + 五区布局壳；地图/角色业务待 Step 4–5，治理迁入待 Step 3 收敛） |
