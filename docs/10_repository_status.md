...
| 成员与 `room_role` | `rooms/membership`, `rooms/permissions` | 房间成员 Tab、角色变更 | `room_members` |
...
数据库迁移：仅 `alembic/versions/20260604_0001_initial_schema.py`，创建 `users`, `rooms`, `feedbacks`, `assets`, `room_members`, `room_join_requests`, `notifications`, `messages`。
...
**身份说明**：下一版 MVP **不新增** `game_role`；DM = `owner` | `manager`，玩家（PL）= `member`。Tabletop 权限见 `08_permission_design.md` §6.4。长期 `game_role` 仍为设计预留，非本版落地项。
...
- **权限分层**：`site_role` 与 `room_role` 已在后端实现；跑团桌面 MVP 用 `room_role` 映射 DM/玩家，不新增 `game_role` 列。
...
| 跑团桌面 MVP（`01` §4.2） | **未开始**（合并原 Phase 2+3 地图/角色/Token/绘制；见模块设计 v0.3） |
...
1. `room_role` Tabletop 权限校验（`08` §6.4）+ 业务 assets（地图 / Token 图）。
2. Table 视口、网格常显、地图底图（上传、锁定、z=0）。
3. Token 层（z=1）与角色 / 怪物数据模型。
4. 绘制层（z=2）、测距工具、橡皮擦与框选删除。
