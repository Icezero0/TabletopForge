...
**身份说明**：跑团 MVP 目标 **新增** `room_members.game_role`（`GM` | `PL` | `OB`），与 `room_role` 分离。Tabletop 权限见 `08` §6.4。代码地基尚无 `game_role` 列。
...
- **权限分层**：`site_role`、`room_role` 已实现；跑团 MVP 待增 `game_role`。
...
| 跑团桌面 MVP（`01` §4.2） | **未开始**（含 `game_role`、角色稳定/状态分层；不测距；Campaign 后续） |
...
1. 迁移 `game_role` + Tabletop 权限（`08` §6.4）+ 业务 assets。
