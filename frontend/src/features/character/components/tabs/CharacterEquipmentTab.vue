...
function removeItem(i: number) {
  ...
  editingItems.value = shiftEditSet(editingItems.value, i);
  itemDrafts.value = shiftDraftMap(itemDrafts.value, i);
  if (!wasNew) update("items", localItems.value);
}
function updateItem(i: number, field: keyof Item, value: unknown) {
  ...
  if (!(itemDrafts.value.get(i)?.isNew)) update("items", localItems.value);
}
      ...
      <BaseListItem v-for="(item, i) in localItems" :key="i" dense>
        <div class="item-row">
          ...
          <template v-if="!editingItems.has(i)">
            ...
            <button class="action-btn" @click="startEditItem(i)"><AppIcon :icon="PencilIcon" :size="14" /></button>
          ...
          <template v-else>
            ...
            <div class="item-actions">
              <button class="action-btn confirm-btn" :disabled="!item.name.trim()" @click="confirmItem(i)"><AppIcon :icon="CheckIcon" :size="14" /></button>
              <button class="action-btn cancel-btn" @click="cancelItem(i)"><AppIcon :icon="XMarkIcon" :size="14" /></button>
            </div>
          </template>
...
.cancel-btn { color: var(--c-text-muted); }
.del-btn {
  background: none; border: none; cursor: pointer; color: var(--c-text-muted);
  padding: 4px; border-radius: var(--r-1); display: flex; align-items: center;
  transition: color 0.12s; flex-shrink: 0;
