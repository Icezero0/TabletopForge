...
import { useRoomGovernance } from "@/features/room/composables/useRoomGovernance";
import {
...
import { useTabletopRealtimeEvents } from "@/features/table/composables/useTabletopRealtimeEvents";
...
const governancePresentUserIds = ref<Set<number>>(new Set());
const governanceHasPresenceSnapshot = ref(false);
...
const governance = useRoomGovernance({
  ...
  router,
  t,
  presentUserIds: computed(() => governancePresentUserIds.value),
  hasPresenceSnapshot: computed(() => governanceHasPresenceSnapshot.value),
  onPlayerColorUpdated: (color) => {
...
const {
  ...
  ownerNameByUserId: governanceOwnerNameByUserId,
  playerColorByUserId,
  selfPlayerColor,
  takenPlayerColors,
  requestsLoading,
...
watch(presentUserIds, (ids) => {
  governancePresentUserIds.value = ids;
}, { immediate: true });
watch(() => realtime.hasPresenceSnapshot.value, (hasSnapshot) => {
  governanceHasPresenceSnapshot.value = hasSnapshot;
}, { immediate: true });
...
const ownerNameByUserId = computed(() => {
  const map = new Map(governanceOwnerNameByUserId.value);
