...
注意：不要输出 `token_configs` 或其他指示物配置字段。主要指示物由角色卡的名称、头像、属性、特性、法术、资源与背包数据自动派生；次要指示物由用户在界面中手动创建。
...
| `notes` | string | 备注；没有则 `""` |
| `section` | string | `"common"` 或 `"special"`；可由职业等级确定的资源填 `"common"`，文本中额外写明但不属于通用自动计算的资源填 `"special"` |

规则：
- 若文本明确给出资源次数/上限（例如「荒野形态 2/短休」「引导神力 1/短休」），加入 `resources`。
- 若能由职业等级确定的通用资源，也可以加入并标记 `section: "common"`：生命骰、法术位、邪术师法术位、德鲁伊荒野形态、牧师/圣武士引导神力等。
- 若是文本中额外出现、玩家自行记录或依赖特定来源的资源，标记 `section: "special"`。
...
- 不要添加输出格式之外的顶层字段，尤其不要输出 `token_configs`、主要指示物、次要指示物或 room token 数据。
